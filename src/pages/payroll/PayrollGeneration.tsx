...
import { programProjectsDB } from '../../services/db/indexedDB';
...
const PayrollGeneration = () => {
  ...
  const loadInitialData = async () => {
    try {
      ...
      // Filter for active/ongoing projects
      setProjects(projectsData.filter(p => p.status === 'active' || p.status === 'ongoing'));
      setDistributions([]);
    } catch (error) {
  ...
  const loadPeriodData = async () => {
    try {
      ...
      // No existing allocations since finance module is removed
  ...
  const handleGenerate = async () => {
    try {
      ...
      // Store distributions in local state (finance module removed)
      setDistributions(prev => [...prev, ...distributionsToCreate.map((d, i) => ({ ...d, id: Date.now() + i }))]);

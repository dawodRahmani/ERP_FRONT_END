...
import { governmentOutreachDB } from '../../services/db/indexedDB';
import type { GovernmentOutreachRecord } from '../../types/modules/compliance';
...
const GovernmentOutreach = () => {
  const [outreaches, setOutreaches] = useState<GovernmentOutreachRecord[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [showModal, setShowModal] = useState<boolean>(false);
  const [selectedOutreach, setSelectedOutreach] = useState<GovernmentOutreachRecord | null>(null);
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const data = await governmentOutreachDB.getAll();
      setOutreaches(data);
    } catch (error) {
  ...
  const handleOpenModal = (outreach: GovernmentOutreachRecord | null = null) => {
    if (outreach) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      if (selectedOutreach) {
        await governmentOutreachDB.update(selectedOutreach.id, formData);
      } else {
        await governmentOutreachDB.create(formData);
      }
      setShowModal(false);
      loadData();
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Delete this outreach record?')) {
      await governmentOutreachDB.delete(id);
      loadData();
    }
  };

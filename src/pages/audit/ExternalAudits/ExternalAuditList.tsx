/**
 * External Audits List Page
 *
 * Displays all external audits with filtering and search.
 */

import { useState, useMemo } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { Plus, Search, Filter, Eye, Edit2, Trash2, Check, X, AlertCircle } from 'lucide-react';
import { useExternalAuditsList, useDeleteExternalAudit } from '../../../hooks/audit';
import {
...
const ExternalAuditList: React.FC = () => {
  ...
  const { data: audits = [], isLoading, error } = useExternalAuditsList();
  const deleteMutation = useDeleteExternalAudit();
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      console.error('Failed to delete external audit:', error);
    }
  };
    ...
    return (
      <div className="p-6">
        <div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-4 rounded-lg">
          Error loading external audits: {error.message}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">External Audits</h1>
        <p className="text-gray-600 dark:text-gray-400 mt-1">
          Audits conducted by external audit companies
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by number, type, or company..."
            value={searchTerm}
                  ...
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      {deleteConfirmId === item.id ? (
                        ...
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => navigate(`/compliance/audit/external/${item.id}`)}
                            className="text-gray-600 hover:text-gray-800 dark:text-gray-400"
                            title="View"
                          >
                          ...
                          <button
                            onClick={() => navigate(`/compliance/audit/external/${item.id}?edit=true`)}
...
export default ExternalAuditList;

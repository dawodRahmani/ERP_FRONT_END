...
const FILE_CHECKLIST_ITEMS: ChecklistItem[] = [
  ...
  {
    id: "committee_report",
    name: "Committee Recommendation Report",
    required: true,
  },
  { id: "offer_letter", name: "Signed Offer Letter", required: true },
  {
    id: "sanction_clearance",
    name: "Sanction Clearance Document",
    required: true,
  },
  ...
  },
  {
    ...
    name: "Criminal Clearance Certificate",
    required: true,
  },
...
// Mock employee history data - in production, this would come from an API
const MOCK_EMPLOYEE_HISTORY: EmploymentHistory[] = [
  {
    id: 1,
    employeeId: "VDO-EMP-001",
    employeeName: "Zia Zaheen",
    position: "IT Officer",
    project: "Project A",
    fromDate: "2022-01-01",
    toDate: "2022-11-30",
...
const STATUS_CONFIG: Record<string, { bg: string; text: string }> = {
  ...
  pending_signature: { bg: "bg-yellow-100 dark:bg-yellow-900/30", text: "text-yellow-700 dark:text-yellow-400" },
  signed: { bg: "bg-blue-100 dark:bg-blue-900/30", text: "text-blue-700 dark:text-blue-400" },
  active: { bg: "bg-green-100 dark:bg-green-900/30", text: "text-green-700 dark:text-green-400" },
};

// ==================== HELPER FUNCTIONS ====================

const calculateTotalExperience = (history: EmploymentHistory[]): string => {
  let totalMonths = 0;
...
const Step15Contract: React.FC<StepProps> = ({
  recruitment,
  onAdvance,
  isCurrentStep,
}) => {
  ...
  const [activeTab, setActiveTab] = useState<"contract" | "checklist" | "history">("contract");
  const [editMode, setEditMode] = useState(false);
  const [employeeHistory, setEmployeeHistory] = useState<EmploymentHistory[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        let existingContract = await contractDB.getByRecruitmentId(
          recruitment.id
        );
        ...
        // Load checklist
        let existingChecklist = await checklistDB.getByRecruitmentId(
          recruitment.id
        );
        if (!existingChecklist && existingContract) {
          existingChecklist = await checklistDB.create({
            recruitmentId: recruitment.id,
            items: FILE_CHECKLIST_ITEMS.reduce(
        ...
        // Load employee history (mock data for now)
        // In production, fetch from API based on the candidate
        setEmployeeHistory(MOCK_EMPLOYEE_HISTORY);
      } catch (err) {
    ...
    recruitment.contractType,
  ]);

  const handleContractChange = useCallback(
    (field: keyof Contract, value: string | number) => {
  ...
  const handleChecklistToggle = useCallback(
    async (itemId: string) => {
      ...
      try {
        const existingChecklist = await checklistDB.getByRecruitmentId(
          recruitment.id
        );
        if (existingChecklist) {
  ...
  const handleActivate = useCallback(async () => {
    ...
    if (!allRequired) {
      setError(
        "All required documents must be checked before activating the contract."
      );
      return;
    }
  ...
  }, [contract, checklist, recruitment.id]);
  ...
  const requiredCount = useMemo(
    () => FILE_CHECKLIST_ITEMS.filter((item) => item.required).length,
    []
  );

  const requiredCompleted = useMemo(
    () =>
      FILE_CHECKLIST_ITEMS.filter((item) => item.required && checklist[item.id])
        .length,
    [checklist]
  );

  const totalExperience = useMemo(
    () => calculateTotalExperience(employeeHistory),
    [employeeHistory]
  );
  ...
  //               Please complete all background verification checks before
            ...
            {contract?.status && (
              <span
                className={`px-3 py-1 rounded-full text-sm ${
                  STATUS_CONFIG[contract.status]?.bg || ""
                } ${STATUS_CONFIG[contract.status]?.text || ""}`}
              >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="border-b border-gray-200 dark:border-gray-700">
          <nav className="flex -mb-px">
            ...
            >
              <History className="w-4 h-4" /> Employee History
            </button>
          </nav>
        </div>
          ...
          {activeTab === "contract" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleContractChange("positionTitle", e.target.value)
                    }
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleContractChange("contractType", e.target.value)
                    }
                    disabled={contract?.status !== "draft"}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white disabled:opacity-50"
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleContractChange("department", e.target.value)
                    }
                    disabled={contract?.status !== "draft"}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white disabled:opacity-50"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleContractChange("location", e.target.value)
                    }
                    disabled={contract?.status !== "draft"}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white disabled:opacity-50"
                  />
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    <DollarSign className="w-4 h-4 inline mr-1" /> Monthly Salary
                    (AFN)
                  </label>
                  <input
                    type="number"
                    ...
                    onChange={(e) =>
                      handleContractChange("salary", parseFloat(e.target.value))
                    }
                    disabled={contract?.status !== "draft"}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white disabled:opacity-50"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleContractChange("reportingTo", e.target.value)
                    }
                    disabled={contract?.status !== "draft"}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white disabled:opacity-50"
                  />
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-green-700 dark:text-green-400">
                        The recruitment process is complete. The employee has
                        been onboarded.
                      </p>
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                    Required: {requiredCompleted}/{requiredCount} • Total:{" "}
                    {completedCount}/{FILE_CHECKLIST_ITEMS.length}
                  </p>
                </div>
                <div className="w-32 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${
                        (completedCount / FILE_CHECKLIST_ITEMS.length) * 100
                      }%`,
                    }}
                  />
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      Previous positions held by this employee within VDO
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button className="flex items-center gap-2 px-3 py-1.5 text-sm bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                      <span className="text-lg font-medium text-blue-600 dark:text-blue-400">
                        {employeeHistory[0].employeeName
                          .split(" ")
                          .map((n) => n[0])
                          .join("")}
                      </span>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        Employee ID: {employeeHistory[0].employeeId}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {employeeHistory.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="mt-1 text-gray-500 dark:text-gray-400">
                    This is the employee's first position within the
                    organization.
                  </p>
                </div>
              )}
      ...
      {canEdit && (
        <div className="flex justify-end gap-3">
          ...
          {contract?.status === "signed" && (
            ...
            >
              <CheckCircle className="w-4 h-4" /> Activate Contract & Complete
              Recruitment
            </button>
          )}
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 opacity-90">
                The recruitment process for {contract.positionTitle} has been
                successfully completed. The employee file is ready for handover
                to HR records.
              </p>
            </div>
          </div>

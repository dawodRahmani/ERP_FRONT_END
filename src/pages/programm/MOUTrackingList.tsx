...
import type { MOUTrackingRecord } from "../../types/modules/tracking.ts";

const {
  getAll: getAllMOUTracking,
  delete: deleteMOUTracking,
  search: searchMOUTracking,
...
const MOUTrackingList = () => {
  const [entries, setEntries] = useState<MOUTrackingRecord[]>([]);
  const [filteredEntries, setFilteredEntries] = useState<MOUTrackingRecord[]>(
    []
  );
  ...
  const applyFilters = useCallback(async () => {
    ...
    if (filterAuthority) {
      filtered = filtered.filter(
        (entry) => entry.sectoralAuthority === filterAuthority
      );
    }
  ...
  const handleDelete = async (id: number) => {
    if (
      window.confirm("Are you sure you want to delete this MOU tracking entry?")
    ) {
      try {
  ...
  const getStatusBadgeClass = (status: "project" | "amendment") => {
    const classes = {
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredEntries.length === 0 ? (
                <tr>
                  ...
                  >
                    No MOU tracking entries found. Click "Add MOU Entry" to
                    create one.
                  </td>
                </tr>
              ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusBadgeClass(
                          entry.status as "project" | "amendment"
                        )}`}
                      >

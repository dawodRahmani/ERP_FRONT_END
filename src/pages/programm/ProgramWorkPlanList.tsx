...
import programWorkPlanService from "../../services/db/programWorkPlanService.ts";
import {
  type ProgramWorkPlanRecord,
  type WorkPlanStatus
} from "../../types/modules/tracking";

const ProgramWorkPlanList = () => {
  const [workPlans, setWorkPlans] = useState<ProgramWorkPlanRecord[]>([]);
  const [filteredPlans, setFilteredPlans] = useState<ProgramWorkPlanRecord[]>(
    []
  );
  ...
  }, [filterDonor, filterProject, filterStatus, search, searchTerm, workPlans]);
  ...
  }, [getUniqueDonors, getUniqueProjects]);
  ...
  const getStatusBadgeClass = (status: WorkPlanStatus) => {
    const classes = {
  ...
  useEffect(() => {
    loadWorkPlans();
    loadFilterOptions();
  }, [loadFilterOptions, loadWorkPlans]);

...
const LeaveBalances = () => {
  const [balances, setBalances] = useState<EmployeeLeaveBalanceRecord[]>([]);
  ...
  const calculateAvailable = (balance) => {
    return leaveBalancesDB.calculateAvailable(balance);
  };
  ...
  const handleAdjustment = async (e) => {
    ...
    try {
      await leaveAdjustmentsDB.create({
      ...
      const adjustments = await leaveAdjustmentsDB.getPending();
      const latest = adjustments[adjustments.length - 1];
      if (latest) {
        await leaveAdjustmentsDB.approve(latest.id, "HR Admin");
      }
  ...
  const handleInitialize = async (e) => {
    ...
    try {
      await leaveBalancesDB.initializeForEmployee(
        Number(initEmployeeId),
        filterYear
      );
  ...
  const filteredBalances = balances.filter((balance) => {
    ...
    const matchesEmployee =
      !filterEmployee || balance.employeeId === Number(filterEmployee);
    const matchesLeaveType =
      !filterLeaveType || balance.leaveTypeId === Number(filterLeaveType);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md m-4">
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                {getEmployeeName(selectedBalance.employeeId)} -{" "}
                {getLeaveTypeName(selectedBalance.leaveTypeId)}
              </p>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md m-4">
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Create leave balances for an employee based on active leave
                types
              </p>
            </div>
            <form onSubmit={handleInitialize} className="p-6 space-y-4">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg">
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  This will create leave balances for all active leave types
                  based on their default entitlements.
                </p>
              </div>
              <div className="flex justify-end gap-3 pt-4">
                <button

...
import type { DepartmentRecord, PositionRecord } from '../../types/modules/coreHR';
...
const Recruitment = () => {
  ...
  // Requisition state
  const [requisitions, setRequisitions] = useState<SRFRecord[]>([]);
  const [showRequisitionModal, setShowRequisitionModal] = useState<boolean>(false);
  const [editingRequisition, setEditingRequisition] = useState<SRFRecord | null>(null);
  const [showDeleteConfirm, setShowDeleteConfirm] = useState<number | null>(null);

  // Announcement state
  const [announcements, setAnnouncements] = useState<VacancyAnnouncementRecord[]>([]);
  const [showAnnouncementModal, setShowAnnouncementModal] = useState<boolean>(false);
  const [editingAnnouncement, setEditingAnnouncement] = useState<VacancyAnnouncementRecord | null>(null);
  const [selectedAnnouncementForCandidate, setSelectedAnnouncementForCandidate] = useState<VacancyAnnouncementRecord | null>(null);
  ...
  const [selectedRequisitionForCandidate, setSelectedRequisitionForCandidate] = useState<SRFRecord | null>(null);

  // Interview state
  const [interviews, setInterviews] = useState<RecruitmentInterviewRecord[]>([]);
  const [showInterviewModal, setShowInterviewModal] = useState<boolean>(false);
  const [editingInterview, setEditingInterview] = useState<RecruitmentInterviewRecord | null>(null);

  // Offer state
  const [offers, setOffers] = useState<OfferLetterRecord[]>([]);
  const [showOfferModal, setShowOfferModal] = useState<boolean>(false);
  const [editingOffer, setEditingOffer] = useState<OfferLetterRecord | null>(null);
  ...
  const handleSaveRequisition = async () => {
    ...
    try {
      if (editingRequisition) {
        await srfDB.update(editingRequisition.id, requisitionForm);
      } else {
        await srfDB.create(requisitionForm);
      }
  ...
  const handleSaveAnnouncement = async () => {
    ...
    try {
      if (editingAnnouncement) {
        await vacancyDB.update(editingAnnouncement.id, announcementForm);
      } else {
        await vacancyDB.create(announcementForm);
      }
  ...
  const handleSaveOffer = async () => {
    ...
    try {
      if (editingOffer) {
        await offerDB.update(editingOffer.id, offerForm);
      } else {
        await offerDB.create(offerForm);
      }
